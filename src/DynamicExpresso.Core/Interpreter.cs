...
namespace DynamicExpresso
{
    ...
    public class Interpreter
    {
        ...
        {
            _expressionText = expressionText.Replace("?) Null", "?)Null");
            _expressionTextLength = _expressionText.Length;

            if (_expressionText.IndexOf("?)null") > -1)
            {
                expressionText = EditNullType(typeof(void), parameters);
            }
        ...
        private string EditNullType(Type expressionType, params Parameter[] parameters)
        {
            while (_expressionText.IndexOf("?)null") > -1)
            {
                var pos = _expressionText.IndexOf("?)null");
                var start = GetStartPosition(pos);
                var end = GetEndPosition(pos);

                var conditional = _expressionText.Substring(start, end - start);

                var result=Parse(conditional, expressionType, parameters).Invoke(parameters);

                if (conditional.Contains("=") || conditional.Contains(">") || conditional.Contains("<"))
                {
                    var boolResult = result.ToString() == "1";
                    _expressionText = _expressionText.Replace(conditional," "+ boolResult.ToString().ToLower()+" ");

                }
                else
                {
                    _expressionText = _expressionText.Replace(conditional, result.ToString());
                }
        ...
        private int GetStartPosition(int pos)
        {
            SetTextPos(pos - 1);

            while (_parsePosition > 0)
            {
                switch (_parseChar)
                {
        ...
        private int GetEndPosition(int pos)
        {
            SetTextPos(pos + 6);

            while (_parsePosition < _expressionTextLength - 1)
            {
                switch (_parseChar)
                {
