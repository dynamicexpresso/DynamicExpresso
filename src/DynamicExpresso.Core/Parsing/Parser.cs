    ...
    internal class Parser
    {
        ...
        private Expression ParseUnary()
        {
            if (_token.id == TokenId.Minus || _token.id == TokenId.Exclamation || _token.id == TokenId.Plus)
            {
                var op = _token;
                NextToken();
                if (_token.id == TokenId.IntegerLiteral || _token.id == TokenId.HexLiteral ||
        ...
        private Expression ParseHexLiteral()
        {
            ValidateToken(TokenId.HexLiteral);
            var text = _token.text;
            if (text[0] != '-')
            {
                if (!ulong.TryParse(text, NumberStyles.HexNumber, ParseCulture, out ulong value))
                    throw CreateParseException(_token.pos, ErrorMessages.InvalidHexLiteral, text);
            ...
            {
                if (!long.TryParse(text, NumberStyles.HexNumber, ParseCulture, out long value))
                    throw CreateParseException(_token.pos, ErrorMessages.InvalidHexLiteral, text);
        ...
        private void NextToken()
        {
            ...
            switch (_parseChar)
            {
                ...
                default:
                    ...
                    if (char.IsDigit(_parseChar) || IsHex(_parseChar))
                    {
                        ...
                        do
                        {
                            ...
                            NextChar();
                        } while (char.IsDigit(_parseChar)|| IsHex(_parseChar));
